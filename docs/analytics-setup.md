...
create table if not exists public.carts (
  ...
  check_in date, check_out date, nights int, adults int, children int, infants int,
...
begin
  insert into public.carts as c (
    ...
    nullif(p->'stay'->>'nights','')::int, nullif(p->'stay'->>'adults','')::int,
    nullif(p->'stay'->>'children','')::int, nullif(p->'stay'->>'infants','')::int,
  ...
  on conflict (cart_id) do update set
    ...
    infants              = coalesce(excluded.infants, c.infants),
